    ...
    <div className="space-y-4">
      <div className="rounded-2xl border border-white/10 bg-slate-900/80 p-6 shadow-lg">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <p className="text-xs uppercase tracking-[0.2em] text-slate-400">График смен</p>
            <h1 className="text-2xl font-semibold text-white">Планирование и исключения</h1>
            <p className="text-sm text-slate-300">Диапазон дат, смены, отпуска/больничные/донорские.</p>
          </div>
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-2">
          <div className="rounded-xl border border-white/10 bg-slate-900/70 p-4 text-sm text-slate-200">
            <p className="text-[11px] uppercase tracking-[0.2em] text-slate-400">Добавить смену</p>
            <div className="mt-2 grid gap-2 sm:grid-cols-2">
              <label className="text-xs text-slate-400">
                ...
                <input
                  ...
                  className="mt-1 w-full rounded-lg border border-white/10 bg-slate-950/60 px-2 py-1 text-sm text-white"
                  placeholder="123"
                />
              </label>
              <label className="text-xs text-slate-400">
            ...
            <label className="mt-2 block text-xs text-slate-400">
              ...
              <input
                ...
                className="mt-1 w-full rounded-lg border border-white/10 bg-slate-950/60 px-3 py-2 text-sm text-white"
            ...
            <button
              ...
              disabled={saving}
              className="mt-3 rounded-full bg-sky-500 px-4 py-2 text-sm font-semibold text-slate-950 transition hover:bg-sky-400 disabled:cursor-not-allowed disabled:opacity-70"
            >
          ...
          <div className="rounded-xl border border-white/10 bg-slate-900/70 p-4 text-sm text-slate-200">
            <p className="text-[11px] uppercase tracking-[0.2em] text-slate-400">Исключение / отсутствие</p>
            <div className="mt-2 grid gap-2 sm:grid-cols-2">
              <label className="text-xs text-slate-400">
                ID сотрудника
                <input
            ...
            <button
              ...
              className="mt-3 rounded-full bg-emerald-500 px-4 py-2 text-sm font-semibold text-slate-950 transition hover:bg-emerald-400 disabled:cursor-not-allowed disabled:opacity-70"
            >
      ...
      <div className="rounded-2xl border border-white/10 bg-slate-900/80 p-6 text-sm text-slate-200 shadow-lg">
        <div className="flex items-center justify-between gap-3">
          <div>
            <p className="text-xs uppercase tracking-[0.2em] text-slate-400">Диапазон</p>
        ...
        <div className="mt-4 space-y-3">
          {grouped.map((day) => (
            <div key={day.date} className="rounded-xl border border-white/5 bg-white/5 p-3">
              <div className="flex items-center justify-between text-xs text-slate-300">
                <span className="text-sm font-semibold text-white">{formatDate(day.date)}</span>
                <span className="rounded-full border border-white/10 bg-sky-500/10 px-2 py-0.5 text-[11px] text-sky-100">
                  {day.list.length} смен
                </span>
              </div>
              <div className="mt-2 space-y-2">
                {day.list.map((row) => (
                  <div key={row.id} className="rounded-lg border border-white/5 bg-slate-900/80 px-3 py-2">
                    <div className="flex flex-wrap items-center justify-between gap-2 text-xs text-slate-400">
                      <span className="font-semibold text-white">{employeeLabel(row.employees)}</span>
                      <span>
                        {row.start_time || '—'} — {row.end_time || '—'} · {row.planned_hours || '—'} ч
                      </span>
                    </div>
                    <div className="flex flex-wrap items-center gap-2 text-[11px] text-slate-400">
                      {row.unit && <span className="rounded-full border border-white/10 px-2 py-0.5">{row.unit}</span>}
                      {row.source && (
                        <span className="rounded-full border border-white/10 bg-white/5 px-2 py-0.5 text-slate-300">
                          {row.source}
                        </span>
                      )}
                      {row.note && <span className="text-slate-300">{row.note}</span>}
                    </div>
          ...
          {!grouped.length && !loading && <p className="text-xs text-slate-400">Нет данных за выбранный период.</p>}
        </div>
      </div>
    </div>
  )

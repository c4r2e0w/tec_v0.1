...
function StartPage() {
  ...
  return (
    <div className="space-y-6">
      <div className="ios-hero rounded-[2rem] border p-6 text-white sm:p-8">
        <div className="absolute inset-x-10 top-0 h-px bg-gradient-to-r from-transparent via-white/35 to-transparent" />
        <div className="absolute -right-10 bottom-[-4rem] h-40 w-40 rounded-full bg-white/10 blur-3xl" />
        <p className="relative z-10 text-xs uppercase tracking-[0.3em] text-emerald-100/70">Смена · Инфопоток</p>
        <div className="mt-4 flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
          <div className="relative z-10 space-y-3">
            <h2 className="text-3xl font-semibold leading-tight tracking-[-0.03em]">Привет, команда УИ-ТЭЦ</h2>
            ...
            <div className="relative z-10 grid grid-cols-1 gap-3 text-sm sm:grid-cols-2">
              <div className="ios-panel-soft rounded-[1.6rem] border border-white/10 p-4">
        ...
        <div className="relative z-10 mt-6 flex flex-wrap gap-3">
          <Link to="/rounds/today" className="ios-chip rounded-full px-5 py-2 text-sm text-white transition hover:border-amber-300 hover:bg-white/10 hover:text-white">
            Сегодняшний обход
          </Link>
          <Link to="/rounds/history" className="ios-chip rounded-full px-5 py-2 text-sm text-white transition hover:border-amber-300 hover:bg-white/10 hover:text-white">
            История обходов
          </Link>
        </div>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <div className="ios-panel rounded-[1.75rem] border border-border p-6">
          ...
          <div className="mt-4 space-y-3">
            {updates.map((update) => (
              <div
                key={update.title}
                className="ios-panel-soft flex items-start gap-3 rounded-[1.25rem] border border-border p-4 transition hover:border-accent/40"
              >
        ...
        <div className="ios-panel rounded-[1.75rem] border border-border p-6">
          ...
          <div className="mt-4 grid gap-3">
            {[
              ...
              <div
                key={task.title}
                className="ios-panel-soft rounded-[1.25rem] border border-border p-4 transition hover:border-emerald-400/40"
              >
      ...
      <div className="ios-panel rounded-[1.75rem] border border-border p-6">
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-3">
          {ideas.map((idea) => (
            <div
              key={idea.text}
              className="ios-panel-soft space-y-3 rounded-[1.25rem] border border-border p-4 transition hover:border-accent/40"
            >
      ...
      <div className="ios-panel rounded-[1.75rem] border border-border p-5 text-sm text-dark">
        <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          <div>
        ...
        {employeeProfile.employee && (
          <div className="mt-3 grid gap-3 md:grid-cols-3">
            <div className="ios-panel-soft rounded-[1.2rem] border border-border p-3">
      ...
      <div className="ios-panel rounded-[1.75rem] border border-border p-5 text-xs text-grayText">
      ...
      <div className="ios-panel rounded-[1.75rem] border border-border p-5 text-sm text-dark">
        ...
        <div className="mt-3 grid gap-2 md:grid-cols-2">
          {employees.map((emp) => {
            ...
            return (
              <div key={emp.id} className="ios-panel-soft rounded-[1.2rem] border border-border p-3">
          ...
          {!loadingEmployees && !employeesError && employees.length === 0 && (
            <div className="ios-panel-soft rounded-[1.2rem] border border-dashed border-border p-3 text-xs text-grayText">
